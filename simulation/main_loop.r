...
obtain_glex_objs <- function(dataset, model, cov_args, only_emp_p = F) {
  ...
  glex_objs <- list(true_p = glex_true_p, TreeSHAP = glex_treeshap, emp_p = glex_emp_p)
  if (!only_emp_p) {
    ...
    glex_objs <- c(glex_objs, list(emp_p50 = glex_emp_p50, emp_p100 = glex_emp_p100))

    if (nrow(x) > 500) {
      emp_p500 <- function(...) probFunctionEmp_(..., x[1:500, ])
      glex_emp_p500 <- glex(model, x, probFunction = emp_p500)
      glex_objs <- c(glex_objs, list(emp_p500 = glex_emp_p500))
    }
  }
...
simulate_for_N <- function(c, s, N, par = T, B = 10, ...) {
  if (par) {
    future_lapply(N,
    ...
    lapply(N, function(n) simulate_inner(n, c, s, par = F, ...))
  }
}
